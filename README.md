...
- **Rootless UID mapping requires `--userns=keep-id`, and this scaffold hit that gap during initial testing.** Without it, Podman's default rootless mapping sends the invoking host user to container UID 0 (root), not to the container UID matching your actual host UID — meaning the `--build-arg UID/GID` values baked into the image (matching your host user) end up as a *different* identity than what the bind-mounted `workspace/` appears to be owned by inside the container. In practice this showed up as `EACCES: permission denied` when Claude Code tried to write into `/home/claude/workspace`, with `ls -la` inside the container showing the directory owned by `root:root` despite being owned by the actual host user outside it. `--userns=keep-id` (added to `podman run` in `run.sh` and `start.sh`) fixes this by mapping the host user to the *same* UID inside the container instead of to root, which is what the build-arg UID/GID setup was already assuming. This is Podman-specific — Docker's default bridge-mode bind mounts don't have this mapping quirk, since Docker doesn't remap UIDs through a user namespace unless explicitly configured to.
